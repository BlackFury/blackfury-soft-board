...
CodePress = {
	...
	initialize : function() {
		this.detect();
		chars = '|13|32|191|57|48|187|188|33|34|'; // charcodes that trigger syntax highlighting
		cc = '&shy;'; // control char
		if(browser.ff) {
			editor = document.getElementById('ffedt');
			document.designMode = 'on';
			document.addEventListener('keydown', this.keyHandler, true);
			document.addEventListener('click', function() {	CodePress.syntaxHighlight(); }, true);			
			document.body.focus();
		}
		else if(browser.ie) {
		...
		evt = (evt) ? evt : (window.event) ? event : null;
	  	if(evt) {
		    ...
		    if((chars.indexOf('|'+charCode+'|')!=-1) && (!evt.ctrlKey && !evt.altKey)) { // syntax highlighting
				...
				// pasted text should be parsed and highlighted
			}
		}
	},
	...
	syntaxHighlight : function() {
		if(browser.ff) {
			...
			// split big files, highlighting parts of it (FF only for now)
			mid = o.indexOf("&amp;shy;"); 
			if(mid-3000<0) {ini=0;end=6000;}
			else if(mid+3000>o.length) {ini=o.length-6000;end=o.length;}
			else {ini=mid-3000;end=mid+3000;}
			x = z = o.substring(ini,end);
			x = x.replace(/\n/g,'<br>');			
...
languages = { 
	...
	/\{([a-zA-Z0-9_\.]*?)\}/g,'<z>{$1}</z>', // Variables de template
